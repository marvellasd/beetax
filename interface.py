...
import pathlib
import re
from utils.parser import clean_response
import time
import uuid

st.set_page_config(page_title="BeeTax", page_icon="🤖", layout="wide")

from beetax import BeeTax

if "llm_flag" not in st.session_state:
    st.session_state.llm_flag = 0
...
def loadAnswer(prompt):
  ...
  with st.chat_message("assistant"):
    ...
    st.session_state.conversation, st.session_state.llm_flag = BeeTax().run_conversation(prompt, st.session_state.conversation, st.session_state.llm_flag)
    
    # if st.session_state.conversation[-3]['role'] == "assistant":
    #   confirmation_answer = clean_response(st.session_state.conversation[-3]['content'])
    #   message_placeholder.markdown(confirmation_answer)
    #   time.sleep(5)
    answer = clean_response(st.session_state.conversation[-1]['content'])  
